  ...
  "inline-flex items-center justify-center whitespace-nowrap rounded-xl text-sm font-bold ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 uppercase tracking-wide",
  {
    variants: {
      variant: {
        ...
        secondary: "bg-primary text-primary-foreground hover:bg-primary/90 border-accent-foreground border-b-4 active:border-b-0",
...
const DuoButton = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    return (
      <Comp
        className={cn(duoButtonVariants({ variant, size, className }))}
        ref={ref}
        {...props}
      />
    )
...
DuoButton.displayName = "Duo Button"
